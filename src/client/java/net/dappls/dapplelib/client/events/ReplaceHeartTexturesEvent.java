...
public interface ReplaceHeartTexturesEvent {
    Event<ReplaceHeartTexturesEvent> EVENT = EventFactory.createArrayBacked(ReplaceHeartTexturesEvent.class, events -> player -> {
        ...
        for (ReplaceHeartTexturesEvent event : sortedEvents) {
            @Nullable HeartTextureSet textureSet = event.getTextureSet(player);
            if (textureSet != null) {
                return textureSet;
            }
        }
    ...
    @Nullable HeartTextureSet getTextureSet(PlayerEntity player);

    record HeartTextureSet(Identifier fullTexture, Identifier fullBlinkingTexture,
